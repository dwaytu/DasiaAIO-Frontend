...
const UserDashboard: FC<UserDashboardProps> = ({ user, onLogout }) => {
  ...
  const fetchSchedule = async (guardId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/guard-replacement/shifts/${guardId}`)
      if (response.ok) {
        const data = await response.json()
        ...
        <nav className="flex-1 flex flex-col gap-2">
          {[
            ...
            <button
              ...
              onClick={() => handleNavigate(view as typeof activeSection)}
            >
              {label}
            </button>
          ))}
        ...
        <header className="bg-white px-8 py-6 flex justify-between items-center shadow-sm border-b border-gray-200">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 m-0">
              ...
              {activeSection === 'support' && 'Contact Support'}
            </h1>
